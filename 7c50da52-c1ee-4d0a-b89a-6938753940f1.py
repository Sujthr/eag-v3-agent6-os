...
async def _crawl4ai_fetch(url: str, timeout: int = 60) -> dict:
    ...
    import crawl4ai.async_logger as _crawler_log_mod

    # Suppress crawl4ai's Rich logger output so it doesn't corrupt the MCP stdio
    # stream. We replace the AsyncLogger's _log method with a no-op for the
    # duration of the crawl (fd-level redirect caused asyncio transport hangs).
    _orig_log = _crawler_log_mod.AsyncLogger._log
    _crawler_log_mod.AsyncLogger._log = lambda *a, **kw: None
    try:
        async with AsyncWebCrawler(verbose=False) as crawler:
            r = await asyncio.wait_for(crawler.arun(url=url), timeout=timeout)
    finally:
        _crawler_log_mod.AsyncLogger._log = _orig_log
